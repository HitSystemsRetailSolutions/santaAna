...
function comprobarFichaje(fichaje: FichajeInterface) {
  ...
  if (fichaje.infoFichaje == null || fichaje.infoFichaje == undefined) {
    ...
    if (
      fichaje.infoFichaje.idTrabajador == null ||
      fichaje.infoFichaje.idTrabajador == undefined
    ) {
      error = true;
    ...
    if (
      fichaje.infoFichaje.fecha == null ||
      fichaje.infoFichaje.fecha == undefined
    ) {
      error = true;
      mensaje += "fichaje.infoFichaje.fecha incorrecto\n";
    } else {
      if (
        fichaje.infoFichaje.fecha.year == null ||
        fichaje.infoFichaje.fecha.year == undefined
      ) {
        error = true;
        mensaje += "fichaje.infoFichaje.fecha.year incorrecto\n";
      }
      if (
        fichaje.infoFichaje.fecha.month == null ||
        fichaje.infoFichaje.fecha.month == undefined
      ) {
        error = true;
        mensaje += "fichaje.infoFichaje.fecha.month incorrecto\n";
      }
      if (
        fichaje.infoFichaje.fecha.day == null ||
        fichaje.infoFichaje.fecha.day == undefined
      ) {
        error = true;
        mensaje += "fichaje.infoFichaje.fecha.day incorrecto\n";
      }
      if (
        fichaje.infoFichaje.fecha.hours == null ||
        fichaje.infoFichaje.fecha.hours == undefined
      ) {
        error = true;
        mensaje += "fichaje.infoFichaje.fecha.hours incorrecto\n";
      }
      if (
        fichaje.infoFichaje.fecha.minutes == null ||
        fichaje.infoFichaje.fecha.minutes == undefined
      ) {
        error = true;
        mensaje += "fichaje.infoFichaje.fecha.minutes incorrecto\n";
      }
      if (
        fichaje.infoFichaje.fecha.seconds == null ||
        fichaje.infoFichaje.fecha.seconds == undefined
      ) {
        error = true;
    ...
    if (!resParametros.error) {
      if (!resFichaje.error) {
        ...
        const fechaEntrada = new Date(
          fichaje.infoFichaje.fecha.year,
          fichaje.infoFichaje.fecha.month,
          fichaje.infoFichaje.fecha.day,
          fichaje.infoFichaje.fecha.hours,
          fichaje.infoFichaje.fecha.minutes,
          fichaje.infoFichaje.fecha.seconds
        );
        ...
        if (fichaje.tipo === "ENTRADA") {
          ...
          server.emit("resFichajes", {
            error: true,
            fichaje,
            mensaje: fichaje["comentario"],
          });
        }
        ...
        recHit(parametros.database, sqlCompleto)
          .then((res) => {
            if (res.recordset[0].resultado == "OK") {
              if (res.rowsAffected.length > 0) {
                ...
                fichaje["comentario"] =
                  "Error sanPedro fichajes: Caso desconocido sqlCompleta2";
                server.emit("resFichajes", {
                  error: true,
                  fichaje,
                  mensaje: fichaje["comentario"],
              ...
              fichaje["comentario"] =
                "Error sanPedro fichajes: Caso desconocido sqlCompleta";
              server.emit("resFichajes", {
                error: true,
                fichaje,
                mensaje: fichaje["comentario"],
          ...
          .catch((err) => {
            console.log(err);
            fichaje["comentario"] =
              "Error sanPedro fichajes: Error en catch sqlCompleta";
            server.emit("resFichajes", {
              error: true,
              fichaje,
              mensaje: fichaje["comentario"],
