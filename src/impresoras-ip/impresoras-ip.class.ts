  ...
  quienSoy(mac: string): any {
    ...
    return recHit("Hit", sql)
      .then((res) => {
        ...
        return {
          error: true,
          mensaje: "No existe ninguna impresora con esa MAC Address",
        };
      })
      .catch((err) => {
        console.log(err);
  ...
  public async mainForGet(mac: string) {
    try {
      ...
      if (!infoImpresora.error) {
        ...
        const mensaje = await this.pop(
          infoImpresora.info.bbdd,
          infoImpresora.info.nombre,
          mac
        );
        if (!mensaje.error) {
          return mensaje.info;
