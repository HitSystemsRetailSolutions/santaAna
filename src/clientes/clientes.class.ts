...
export class Clientes {
    ...
    return recHit(database, sql)
      .then((res: IResult<any>) => {
        ...
        if (recordset.length == 0) {
          return {
            error: true,
            mensaje: "SanPedro: No existe el cliente: " + idClienteFinal,
          };
        }
          ...
          if (recordset[0].esVIP == 1) {
            ...
            return recHit(
              ...
              .catch((err) => {
                ...
                return {
                  error: true,
                  mensaje: "Error 3: clientes/comprobarVIP",
                };
              });
          }
        }
        ...
        if (res.recordset.length > 0) {
          if (res.recordset[0].resultado == "CREADO") {
            ...
            return {
              error: true,
              mensaje:
                "Error, ya existe un cliente con este nombre o tarjeta cliente",
            };
          } else {
            return {
              error: true,
              mensaje:
                "Error SanPedro clientes.class crearNuevoCliente incontrolado",
            };
          }
        } else {
          return {
            error: true,
            mensaje:
              "Error SanPedro clientes.class crearNuevoCliente incontrolado2",
          };
        }
      })
      .catch((err) => {
        ...
        return {
          error: true,
          mensaje: "Error SanPedro clientes.class crearNuevoCliente CATCH",
        };
      });
  }
  ...
  getClientesEspeciales(database: string, busqueda: string) {
    return recHit(
      database,
      `SELECT Codi as id, Nom as nombre, Nif as nif, Adresa as direccion, Ciutat as ciudad, Cp as cp FROM Clients WHERE Nom like '%${busqueda}%' order by Nom`
    )
      .then((res) => {
        if (res.recordset.length > 0) {
